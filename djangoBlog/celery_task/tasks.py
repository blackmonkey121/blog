...
def send_register_active_email(to_email, username, token):
    ...
    receiver = [to_email]
    html_message = """{}:<br><p style="text-indent: 32px">亲爱的 {} 终于等到你！欢迎注册 xxxxxx ～ ！</p><p><a href="{}">点击激活您的账户！</a><br>{}<br></p><p style="margin-top: 160px; text-align: right">——xxxxx 项目组！</p>
...
def send_update_pwd_email(to_email, username, token):
    URL = 'http://127.0.0.1:8000{}'.format(reverse('user:reset', kwargs={'token': token}))
    subject = '密码重置提示 确认邮件!'
    message = '您的密码重置申请已通过，点击下方链接进行修改'
    sender = develop.EMAIL_FROM
    receiver = [to_email]
    html_message = """{}:<br><p style="text-indent: 32px">{} 这是修改密码的验证链接，如果是你的操作，请点击下面的链接修改密码，如果不是，可能您的账户存在安全风险，请主动修改密码。</p><p> 链接:<a href="{}"> </a><br>{}<br></p><p style="margin-top: 160px; text-align: right">——xxxx 项目组！</p>
""".format(username, username, URL, URL)
